...
def get_new_stock_filter(stock_list, date_list, newly_listed_threshold=120):
    ...
    newly_listed_window = pd.Series(index=stock_list,
                                    data=[rqdatac.get_next_trading_date(listed_date, n=newly_listed_threshold) for
                                          listed_date in listed_date_list])
...
def get_st_filter(stock_list, date_list):
    ...
    st_filter = rqdatac.is_st_stock(stock_list, date_list[0], date_list[-1]).astype('float').reindex(columns=stock_list,
                                                                                                     index=date_list)  # 剔除ST
...
def get_suspended_filter(stock_list, date_list):
    suspended_filter = rqdatac.is_suspended(stock_list, date_list[0], date_list[-1]).astype('float').reindex(
...
def neutralization(factor):  # ,order_book_ids,datetime_period,start_date,end_date
    ...
    for i in datetime_period:
        ...
        cfoa_day_result = pd.DataFrame(
            sm.OLS(y.astype(float), x.astype(float), hasconst=False, missing='drop').fit().resid)
...
def Factor_Return_N_IC(factor, n, Rank_IC=True):
    ...
    if Rank_IC == True:
        rank_ic = close.groupby('date')['change_days', 'factor'].corr(method='spearman').reset_index().set_index(
            ['date'])
        return rank_ic[rank_ic.level_1 == 'factor'][['change_days']]
    else:
        normal_ic = close.groupby('date')['change_days', 'factor'].corr(method='pearson').reset_index().set_index(
            ['date'])
...
def group_5(factor, n):
    ...
    current_return = get_price(stock_list, get_previous_trading_date(start_date, 1, market='cn'), end_date,
                               '1d', 'close', 'pre', False, True).close.unstack('order_book_id').pct_change().dropna(
        axis=0, how='all').stack()
    ...
    for i in range(0, len(date_list_whole), n):
        ...
        group1 = period[period.stock.isin(G1)].set_index(['date', 'stock']).current_renturn.unstack('stock').mean(
            axis=1)
        group2 = period[period.stock.isin(G2)].set_index(['date', 'stock']).current_renturn.unstack('stock').mean(
            axis=1)
        group3 = period[period.stock.isin(G3)].set_index(['date', 'stock']).current_renturn.unstack('stock').mean(
            axis=1)
        group4 = period[period.stock.isin(G4)].set_index(['date', 'stock']).current_renturn.unstack('stock').mean(
            axis=1)
        group5 = period[period.stock.isin(G5)].set_index(['date', 'stock']).current_renturn.unstack('stock').mean(
            axis=1)
