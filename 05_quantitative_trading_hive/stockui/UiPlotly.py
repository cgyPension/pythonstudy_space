...
class UiPlotly:
    ...
    def k_rps_fig(self, df,melt_list,threshold=0,is_plate=True):
        '''
        k线+均线+rps
        :param df: 数据
        ...
        if threshold > 0:
            ...
            for melt, color in zip(melt_list, self.colors):
                fig.add_trace(go.Scatter(x=df['trade_date'], y=df[melt], name=melt, line_color=color,line_width=2,mode='lines+markers',
                marker=dict(color=['#ff0000' if y_i > threshold else color for y_i in df[melt].astype(float)],
                            size=[5 if y_i > threshold else 0 for y_i in df[melt].astype(float)],
        ...
        if is_plate:
            # b板块k线上标记板块三线翻红
            df_plate_rps_red = df[df['is_rps_red']==1]
            fig.add_trace(go.Scatter(x=df_plate_rps_red['trade_date'], y=df_plate_rps_red['low_price'].astype(float)*self.lowStand,mode='markers', name='rps_red', hoverinfo='none', marker=dict(symbol='star', size=12, color='#fd5f00')), row=1, col=1)
            # 话万红上下 star-triangle-up
            # symbol = 'x' symbol = 'triangle-up'
