...
select t1.trade_date,
       ...
       t1.last_sealing_time,
       t1.bomb_sealing_nums,
       t1.zt_tj,
       t1.lx_sealing_nums,
       t3.is_new_g,
       t3.selection_reason,
...
left join stock.ods_stock_strong_pool_di t3
...
where t1.td between '%s' and '%s'
        ...
        spark_df.repartition(1).write.insertInto('stock.dwd_stock_zt_di', overwrite=True)  # 如果执行不存在这个表，会报错
        spark.stop
...
# python /opt/code/pythonstudy_space/05_quantitative_trading_hive/dwd/dwd_stock_zt_di.py all
# python /opt/code/pythonstudy_space/05_quantitative_trading_hive/dwd/dwd_stock_zt_di.py update 20210101 20211231
