...
import back_stockui.UiDataUtils
...
class BlockIndWindow(QtWidgets.QMainWindow, back_stockui.UiDataUtils):
    """
    板块详情
    """
    ...
    def double_clicked_bottom_table_view(self, item):
        ...
        if self.all_model == model:
            ...
            s = df[df['stock_code'] == name].iloc[0]
    ...
    def _init_block_data(self):
        # 查询板块所有成分股
        df = self.get_industry_stock(self._block_name)
    ...
    def _refresh_kline_canvas(self, again_load_data=True):
        ...
        # 是否需要重新加载数据
        if again_load_data:
            self._block_data = sdb.block_stock_daily(self._block_name, self._block_start_date, self._block_end_date)
    ...
    def show_with_data(self, s, warning_value=90):
        code = s['code']
        if self._block_name == code:
            return

        self.block_warning_value = warning_value
        self._block_name = code
        name = s['name']
