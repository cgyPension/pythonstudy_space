...
继承
没有指定父类，python3默认继承object类 最好默认写object可以兼容python2
...
# isinstance 用来判断一个对象是否由指定的类（或父类）实例化出来
...
当with代码块执行完成以后,会自动调用__exit_ 方法
'''
...
# 将数据写入到内存设计到 StringIO和BytesIO两个类
...
# 反序列化：将数据从硬盘加载到内存的过程
# write时，只能写入字符串或者二进制
# 字典、列表、数字等都不能直接写入到文件里I

# json: python里的数据(str/list/tuple/dict/int/float/bool/None)等转换成为对应的 json数据类型
# pickle: python里任意的对象转换成为二进制

# 序列化 dumps 将python数据转换成二进制
#       dump  将python数据转换成二进制，同时保存到指定文件
# 反序列化 loads 将二进制加载成为python数据
#         load  读取文件，并将文件的二进制内容加载成为python
import pickle
names = ['易经','一阳指','胸口碎大石','一夜暴富']
...
pickle用来将数据原封不动的转换成为二进制。但是这个二进制,只能在Python里识别。
