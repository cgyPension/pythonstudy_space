...
from util.CommonUtils import get_process_num, get_code_group, get_code_list, get_concept_plate_group, str_pre
...
def multiprocess_run(engine, process_num):
    ...
    engine.execute("""truncate table ods_dc_stock_concept_plate_df;""")
    for r in result_list:
        ...
        if rl:
            ...
            engine.execute(
                """
                insert ignore into ods_dc_stock_concept_plate_df (stock_code, stock_name, concept_plate)
                                     values (%s, %s, %s);
                """, rl
            )
        else:
            print('rl为空')
    print('ods_dc_stock_concept_plate_df：执行完毕！！！')
